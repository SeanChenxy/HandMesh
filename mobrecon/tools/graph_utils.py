...
import torch
from mobhand.tools.coarsening import coarsen, laplacian, perm_index_reverse, lmax_L, rescale_L
...
def build_heria_graphs(mesh_faces):
    # Build graph
    graph_L = []
    graph_Adj = []
    for face in mesh_faces:
        ...
        L = laplacian(mesh_adj, normalized=True)
        graph_L.append(L)
        graph_Adj.append(mesh_adj)
    ...
    return graph_Adj, graph_L


def sparse_python_to_torch(sp_python, dense=False):
    L = sp_python.tocoo()
